...
def _open_ffmpeg_writer(fps: float, width: int, height: int, frame_count: int, output: Path) -> subprocess.Popen:
...
def _write_video(frames: list[np.ndarray], fps: float, audio: Path, output: Path) -> None:
    if not frames:
        raise RuntimeError("OpenTalking Wav2Lip produced no frames.")
    frames = _pad_frames_to_audio(frames, fps, audio)
    height, width = frames[0].shape[:2]
    video_only = output.with_name(f"{output.stem}.video-only.mp4")
    video_only.unlink(missing_ok=True)
    proc = _open_ffmpeg_writer(fps, width, height, len(frames), video_only)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render a real OpenTalking Wav2Lip video on GPU.")
    parser.add_argument("--avatar-dir", required=True)
    parser.add_argument("--audio", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--postprocess-mode", default="opentalking_improved")
    parser.add_argument("--max-seconds", type=float, default=float(os.getenv("OPENTALKING_WAV2LIP_MAX_SECONDS", "0")))
...
def main() -> int:
    ...
    adapter.set_wav2lip_postprocess_mode(args.postprocess_mode)
    ...
    _log_event("avatar_loaded", fps=float(max(1, int(avatar_state.manifest.fps))))
    ...
    _log_event("features_extracted", seconds=round(time.perf_counter() - started, 3))
    predictions = adapter.infer(features, avatar_state)
    _log_event("inference_done", predictions=len(predictions), seconds=round(time.perf_counter() - started, 3))
    frames = [
